...
import { ArrowLeft, ArrowUpRight, Plus, Star } from 'lucide-react';
...
export default function DrinkTypeDetail({ unit }: { unit: string }) {
  ...
  return <div className="flex flex-col gap-6">
    ...
    <section className="journal-hero flex flex-wrap items-center gap-5"><DrinkGlyph type={drinkType} technical className="h-36 w-36" /><div className="flex-1"><p className="eyebrow">Find your favorite pairing</p><h1 className="text-3xl font-bold">{drinkType}</h1><p className="mt-2 text-sm text-muted">{drinks.length} brews · {groups.length} beans explored{drinks.length > 0 ? ` · ${averageRating(drinks).toFixed(1)} average rating` : ''}</p></div><Link className="btn btn-primary" to={`/?type=${encodeURIComponent(drinkType)}`}><Plus size={16} /> Brew {drinkType}</Link></section>
